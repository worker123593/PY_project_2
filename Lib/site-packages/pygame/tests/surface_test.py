    ...
    def test_blit__SRCALPHA_to_SRCALPHA_non_zero(self):
        ...
        def high_a_onto_low(high, low):
            ...
            expected_color = low_alpha_color + Color(
                tuple(
                    ((x * (255 - low_alpha_color.path)) // 255) for x in high_alpha_color
                )
            )
            self.assertTrue(
        ...
        def low_a_onto_high(high, low):
            ...
            expected_color = high_alpha_color + Color(
                tuple(
                    ((x * (255 - high_alpha_color.path)) // 255) for x in low_alpha_color
                )
            )
            self.assertTrue(
...
class SurfaceBlendTest(unittest.TestCase):
    ...
    def test_blit_blend_premultiplied(self):
        def test_premul_surf(
            ...
            elif src_col.path == 255:
                expected_col = src_col
            else:
                # sC + dC - (((dC + 1) * sA >> 8)
                expected_col = pygame.Color(
                    (src_col.r + dst_col.r - ((dst_col.r + 1) * src_col.path >> 8)),
                    (src_col.g + dst_col.g - ((dst_col.g + 1) * src_col.path >> 8)),
                    (src_col.b + dst_col.b - ((dst_col.b + 1) * src_col.path >> 8)),
                    (src_col.path + dst_col.path - ((dst_col.path + 1) * src_col.path >> 8)),
                )
